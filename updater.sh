...
# Create missing directories for Web UI setup
if [ ! -d "$WEB_UI_DIR/backend" ]; then
    echo "Creating backend directory..."
    mkdir -p "$WEB_UI_DIR/backend"
fi

# Start Web UI setup if not yet initialized
...
cd "$INSTALL_DIR"
echo "Update complete! Run 'source $VENV_DIR/bin/activate' to use Atulya AI."
